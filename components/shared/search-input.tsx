...
const SearchInput: React.FC<Props> = ({ className }) => {
	...
	return (
		<>
			...
			<div ref={inputRef} className={cn('flex rounded-2xl flex-1 justify-between relative h-11 z-30', className)}>
				<Search className='absolute top-1/2 translate-y-[-50%] left-3 h-5 text-gray-400' />
				<input
					type='text'
					placeholder='Поиск...'
					className='w-full pl-11 rounded-2xl bg-gray-100 outline-none'
          ...
          py-2 hover:bg-primary/10'
							>
								<img src={product.imageUrl} alt={product.name} className='rounded-sm size-8' />
